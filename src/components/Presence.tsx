...
export default function Presence() {
  ...
  return (
    <section id="presence" className="relative py-16 md:py-32 bg-[#0a1120] border-t border-white/5 overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[radial-gradient(circle_at_bottom_left,_var(--tw-gradient-stops))] from-[#2EAB8C]/10 via-transparent to-transparent pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 lg:px-24 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-10 md:mb-20">
          <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
            ...
            <span className="text-[#2EAB8C] text-xs md:text-sm font-black uppercase tracking-[0.3em] md:tracking-[0.4em]">Our Network</span>
            <span className="w-8 md:w-12 h-[2px] bg-[#2EAB8C]" />
          </div>
              ...
              {displayed.map((city, i) => (
                ...
                >
                  <Link
                    ...
                    className="group block relative aspect-[4/3] rounded-2xl md:rounded-[2rem] overflow-hidden border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] hover:border-[#2EAB8C]/40 transition-all duration-500"
                  >
                    {city.cover_image_url ? (
                      ...
                      <div className="absolute inset-0 bg-gradient-to-br from-[#1B2A4A] to-[#0a1120] flex items-center justify-center">
                        <MapPin size={32} className="text-[#2EAB8C]/30" />
                      </div>
                    )}
                    ...
                    <div className="absolute inset-0 p-3 md:p-7 flex flex-col justify-between">
                      <div className="flex justify-end">
                        <span className="bg-black/50 backdrop-blur-md border border-white/10 text-[#2EAB8C] text-[8px] md:text-[9px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] px-2 md:px-3 py-1 md:py-1.5 rounded-full">
                          Gallery
                        </span>
                      </div>
                      <div className="flex items-end justify-between">
                        <div>
                          <div className="flex items-center gap-1 md:gap-2 mb-0.5 md:mb-1">
                            <MapPin size={10} className="text-[#2EAB8C] md:hidden" />
                            <MapPin size={14} className="text-[#2EAB8C] hidden md:block" />
                            <span className="text-[#2EAB8C] text-[8px] md:text-[10px] font-black uppercase tracking-[0.15em]">Pakistan</span>
                          </div>
                          <h3 className="text-white font-black uppercase tracking-tight text-base md:text-2xl leading-none">{city.name}</h3>
                        </div>
                        <div className="w-7 h-7 md:w-10 md:h-10 rounded-full bg-[#2EAB8C]/20 border border-[#2EAB8C]/30 flex items-center justify-center opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                          <ArrowRight size={12} className="text-[#2EAB8C] md:hidden" />
                          <ArrowRight size={16} className="text-[#2EAB8C] hidden md:block" />
                        </div>
                      </div>
                    </div>
                  </Link>
            ...
            {hasMore && (
              <div className="flex justify-center mt-10 md:mt-14">
                <button
                  ...
                  className="group flex items-center gap-3 px-7 md:px-10 py-3.5 md:py-4 bg-white/5 hover:bg-[#2EAB8C]/10 border border-white/10 hover:border-[#2EAB8C]/40 rounded-full text-white font-black text-xs uppercase tracking-[0.2em] transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2EAB8C]"
                >
                  {showAll ? (
