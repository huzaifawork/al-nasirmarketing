...
export default function Footer() {
  return (
    <footer className="bg-[#050A15] border-t border-white/10 pt-14 md:pt-24 pb-8 md:pb-10 relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-[1000px] h-px bg-gradient-to-r from-transparent via-[#2EAB8C] to-transparent opacity-50" />
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-[#2EAB8C] blur-[150px] opacity-10 rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-12 lg:px-24 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-12 gap-10 md:gap-16 lg:gap-8 mb-12 md:mb-20">
          ...
          <div className="col-span-2 md:col-span-2 lg:col-span-4 flex flex-col pr-0 md:pr-8">
            ...
            <div className="flex gap-3">
              {[{Icon: FaLinkedinIn, label: 'LinkedIn'}, {Icon: FaInstagram, label: 'Instagram'}, {Icon: FaYoutube, label: 'YouTube'}, {Icon: FaFacebookF, label: 'Facebook'}].map(({ Icon, label }, i) => (
                <a key={i} href="#" aria-label={label} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-[#2EAB8C] transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2EAB8C]">
                  <Icon size={16} />
                </a>
              ))}
            </div>
          ...
          <div className="col-span-1 lg:col-span-2">
            ...
            <ul className="space-y-3 md:space-y-4">
              {['Home', 'About Us', 'Clients', 'Presence', 'Team'].map((link) => (
                <li key={link}>
                  <Link href={`#${link.toLowerCase().replace(' ', '')}`} className="text-gray-400 text-sm hover:text-[#2EAB8C] transition-colors flex items-center group font-medium">
                    <span className="w-0 group-hover:w-3 h-[2px] bg-[#2EAB8C] transition-all mr-0 group-hover:mr-2" />
                    {link}
                  </Link>
                </li>
              ))}
          ...
          <div className="col-span-1 lg:col-span-3">
            ...
            <ul className="space-y-3 md:space-y-4">
              {['Billboard Ads', 'SMD / LED Screens', 'Social Media', 'UGC Content', 'AI Strategy'].map((service) => (
                <li key={service}>
                  <Link href="#services" className="text-gray-400 text-sm hover:text-[#2EAB8C] transition-colors flex items-center group font-medium">
                    <span className="w-0 group-hover:w-3 h-[2px] bg-[#2EAB8C] transition-all mr-0 group-hover:mr-2" />
                    {service}
                  </Link>
                </li>
          ...
          <div className="col-span-2 md:col-span-2 lg:col-span-3">
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="flex items-start gap-3">
                <MapPin className="text-[#2EAB8C] shrink-0 mt-0.5" size={16} />
                <div>
              ...
              <div className="flex items-start gap-3">
                <Phone className="text-[#2EAB8C] shrink-0 mt-0.5" size={16} />
                <div>
                  <p className="text-white text-xs font-bold uppercase tracking-wider mb-0.5">Phone</p>
                  <p className="text-gray-400 text-xs">+92 300 000 0000</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Mail className="text-[#2EAB8C] shrink-0 mt-0.5" size={16} />
                <div>
                  <p className="text-white text-xs font-bold uppercase tracking-wider mb-0.5">Email</p>
                  <a href="mailto:contact@alnasir.pk" className="text-gray-400 text-xs hover:text-white transition-colors">contact@alnasir.pk</a>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="border-t border-white/10 pt-6 md:pt-10 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-gray-500 text-[10px] font-bold uppercase tracking-[0.2em] text-center sm:text-left">
            &copy; {new Date().getFullYear()} AL-NASIR ADVERTISING. All rights reserved.
          </p>
